...
class Phone_book:
    ...
    def format_number(self, data):
        ...
        pattern_number = r'(\+7|8)\s*\(*(\d{3})\)*[\s|-]*(\d{3})[-]*(\d{2})[-]*(\d{2})((\s*)\(*(доб.)\s*(\d{4})\)*)?'
        number_new_format = r'+7(\2)\3-\4-\5\7\8\9'

        for text in data:
            text_search = ','.join(text)
            format_text = re.sub(pattern_number, number_new_format, text_search)
    ...
    def format_name(self, data):
        self.data = data
        self.new_contacts_list = []
        pattern_name = r'(^[А-ё]+)(\s*)(\,?)([А-ё]+)(\s*)(\,?)([А-ё]+)?(\,?)(\,?)(\,?)'
        name_new_format = r'\1,\4\6\9\7\8'

        for text in data:
            text_search = ','.join(text)
            format_text = re.sub(pattern_name, name_new_format, text_search)
            format_list = format_text.split(',')
            self.new_contacts_list.append(format_list)
        return self.new_contacts_list
